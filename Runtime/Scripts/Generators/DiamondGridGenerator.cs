...
{
    public class DiamondGridGenerator : INodeMapGenerator
    {
        public NodeMap Generate(NodeMapSettings settings)
        {
            ...
            for (int i = 0; i < translatedPoints.Count; i++)
            {
                for (int j = i + 1; j < translatedPoints.Count; j++)
                {
                    ...
                    {
                        nodeMap.Paths.Add(new Path { Source = nodeMap.Vertices[i], Destination = nodeMap.Vertices[j] });
                    }
                }
            }

            nodeMap.Start = nodeMap.Vertices.First();
            nodeMap.End = nodeMap.Vertices.Last();
